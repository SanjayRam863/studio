...
const formSchema = z.object({
  medicalConditions: z.string().min(1, "Please enter at least one condition or 'None'."),
  calorieNeeds: z.coerce.number().min(500, "Calorie needs must be at least 500.").max(10000, "Calorie needs seem too high."),
});
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      List any medical conditions, separated by commas. Type 'None' if you have no conditions.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <Tabs defaultValue="plan">
              ...
              <TabsContent value="plan" className="mt-4">
                <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">Total Calories: <Badge variant="secondary">{dietPlan.totalCalories}</Badge></p>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {dietPlan.dietPlan.map((meal, index) => (
                            <Card key={index}>
                                <CardHeader>
                                    <CardTitle>{meal.name}</CardTitle>
                                    <p className="text-sm font-medium text-primary">{meal.calories} calories</p>
                                </CardHeader>
                                <CardContent>
              ...
              <TabsContent value="notes" className="mt-4">
                <p className="text-sm text-muted-foreground whitespace-pre-wrap">{dietPlan.notes}</p>
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
